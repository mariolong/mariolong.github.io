...
.. updated: 2018-10-01 12:17:43 UTC+08:00
.. description:
#+END_COMMENT
#+OPTIONS: num:t
#+TITLE: Install Arch linux (2017)
...
** 時間調整
<<set-time-on-windows>>為了與 windows 雙重開機 (dual boot)，最好做以下動作：
...
#+BEGIN_SRC sh
yaourt -S ttf-tw ttf-dejavu ttf-liberation ttf-hanazono ttf-symbola
...
*** install lxqt
雖然目前只用 ~lxqt-policykit-agent~ 和 ~lxappearance~，還是 lxqt 全裝，
省事。
...
#+BEGIN_SRC sh
yaourt -S breeze{,-gtk} flat-remix-git lxappearance adwaita-icon-theme
#+END_SRC
...
** 為了與 windows 能雙系統共存，有四件事要注意：
*** 先裝 windows 再裝 Linux 比較省事
*** disable UEFI Secure Boot on BIOS
這個要在安裝 windows 前先做比較合理

*** turn off Fast Start-up
參考 [[https://www.tenforums.com/tutorials/4189-turn-off-fast-startup-windows-10-a.html][Turn On or Off Fast Startup in Windows 10 | Windows 10 Tutorials]]
原則上就是找到「控制台」->「電源選項」->「其他電源設定」

*** set systemc time
[[set-time-on-windows][如上述]]
